	$.ajax(
			{
				type: "GET",
				url: 'http://localhost:8080/rest/topo/nodes',
				data: "{}",
...
//					var edgeCount = 12;
					...
					var nodeCount = getNodes.length;


					for (var i = 0; i < nodeCount; i++) {
						demoNodes.push({
							data: {
								id: getNodes[i].nodeID,
					...
					for (var i = 0; i < nodeCount-1; i++) {
						demoEdges.push({
							data: {
								id: "e" + (i * 2),
								source: getNodes[i].nodeID,
								target: getNodes[i+1].nodeID,
								weight: 30
							}
						});
